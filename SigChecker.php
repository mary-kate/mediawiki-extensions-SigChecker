...
function SigCheckerInitBlacklist() {
	        global $wgUnsigBlacklist;
	        if( isset( $wgUnsigBlacklist ) && $wgUnsigBlacklist ) return;
...
function SigCheckEditFilterMerged( $editor, $text, &$error, $summary )
{
  ...
  if ( !$wgUnsigBlacklist->isBlacklisted($wgUser->getName()) ) return true;
  if (!$editor->getArticle()->getTitle()->isTalkPage()) return true;
